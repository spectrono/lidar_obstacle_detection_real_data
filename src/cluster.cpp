...
{
	std::cout << "searching nearby points for index: " << index << std::endl;
	...
	std::cout << "... found points count: " << nearby_points.size() << std::endl;
	for (auto nearby_point_index : nearby_points)
	{
		if (!isPointProcessed[nearby_point_index])
		{
			proximityBasedClusterAggregation(nearby_point_index, isPointProcessed, cluster, points, tree, distanceTol);
		}
		else
		{
			std::cout << "skipping index: " << nearby_point_index << std::endl;
		}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for (int index = 0; index < points_count; ++index)
	{
		if (!isPointProcessed[index])
		{
			...
			proximityBasedClusterAggregation(index, isPointProcessed, cluster, points, tree, distanceTol);
